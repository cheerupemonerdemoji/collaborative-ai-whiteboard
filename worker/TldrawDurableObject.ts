...
export class TldrawDurableObject extends DurableObject {
	...
	async checkAiRateLimit(subject: string): Promise<boolean> {
		const key = `ai-rate:${subject.slice(0, 120)}`
		const now = Date.now()
		const recent = (await this.ctx.storage.get<number[]>(key) ?? []).filter((time) => now - time < 60_000)
